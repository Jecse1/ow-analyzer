        ...
        <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', color: theme.text }}>
            <div style={{ marginBottom: '32px' }}>
                <h1 style={{ fontSize: '32px', fontWeight: '900', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <AlertOctagon size={36} color={DANGER_COLOR} /> 퍼뎃 분석 (First Death Stats)
                </h1>
                <p style={{ color: theme.textSub, marginTop: '8px' }}>선수들의 생존 능력과, 주로 누구에게/어떤 스킬에 먼저 죽었는지를 상세히 분석합니다.</p>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: theme.surfaceHighlight }}>
                        <tr>
                            ...
                            <th style={{ padding: '16px', textAlign: 'right', fontSize: '13px', color: theme.textSub, cursor:'pointer' }} onClick={() => handleSort('firstDeaths')}>퍼뎃 횟수 <ArrowUpDown size={12} style={{display:'inline'}}/></th>
                            <th style={{ padding: '16px', textAlign: 'right', fontSize: '13px', color: DANGER_COLOR, cursor:'pointer', fontWeight:'900' }} onClick={() => handleSort('rate')}>퍼뎃 비율 (%) <ArrowUpDown size={12} style={{display:'inline'}}/></th>
                            ...
                            return (
                                <React.Fragment key={p.name}>
                                    <tr onClick={() => toggleExpand(p.name)} style={{ background: isExpanded ? theme.surfaceHighlight : rowBg, borderBottom: isExpanded ? 'none' : `1px solid ${theme.border}40`, transition: 'background 0.2s', cursor: 'pointer' }} onMouseOver={e=>e.currentTarget.style.background=theme.surfaceHighlight} onMouseOut={e=>e.currentTarget.style.background=isExpanded ? theme.surfaceHighlight : rowBg}>
                                        ...
                                        <td style={{ padding: '16px', textAlign: 'right', fontWeight: 'bold' }}>{p.firstDeaths}회</td>
                                        ...
                                        <tr style={{ background: theme.surfaceHighlight, borderBottom: `2px solid ${theme.border}` }}>
                                            <td colSpan="7" style={{ padding: '0 24px 24px 24px' }}>
                                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', background: theme.bg, padding: '20px', borderRadius: '12px', border: `1px solid ${theme.border}40` }}>
                                                    <div>
                                                        <h4 style={{ fontSize: '13px', color: theme.textSub, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Skull size={14} color={DANGER_COLOR}/> 천적 (Player)</h4>
                                                        {getTopN(p.deathLogs, 'killerName', 5).map((target, i) => {
                                                            ...
                                                            return (
                                                                <div key={i} style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', padding: '6px 8px', marginBottom: '4px', borderRadius: '4px', background: 'rgba(255,255,255,0.03)', overflow: 'hidden' }}>
                                                                    ...
                                                                    <span style={{ position: 'relative', zIndex: 1, fontSize: '13px', fontWeight: 'bold', color: DANGER_COLOR }}>{target.count}회 <span style={{fontSize:'10px', color:theme.textSub, fontWeight:'normal'}}>({pct.toFixed(0)}%)</span></span>
                                                                </div>
                                                            )
                                                        })}
                                                    </div>
                                                    <div>
                                                        <h4 style={{ fontSize: '13px', color: theme.textSub, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Shield size={14} color={DANGER_COLOR}/> 경계 대상 영웅 (Hero)</h4>
                                                    ...
                                                    <div>
                                                        <h4 style={{ fontSize: '13px', color: theme.textSub, marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}><Sword size={14} color={DANGER_COLOR}/> 치명적 스킬 (누구의 스킬을 맞았는가)</h4>
