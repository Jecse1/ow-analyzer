...
def build_fight_summaries(
    kill_events: List[Dict[str, Any]], team1: str, team2: str, quiet_gap_sec: int = FIGHT_QUIET_GAP_SEC
):
    kills = []
...
def parse_overwatch_log(log_text: str, custom_t1: str = None, custom_t2: str = None):
    ...
    def map_team(t_raw):
        t = t_raw.strip()
        if custom_t1 and t == log_t1:
            return custom_t1
        if custom_t2 and t == log_t2:
            return custom_t2
        if custom_t1 and t in ["Team 1", "1팀"]:
            return custom_t1
        if custom_t2 and t in ["Team 2", "2팀"]:
            return custom_t2
        return t
    ...
    for line in lines:
        ...
        if ",match_start," in clean_line:
            ...
            try:
                ...
                if first_team_name and second_team_name:
                    ...
                    for t in tail:
                        ...
                        if nt == n1:
                            match_end_winner = first_team_name
                            break
                        if nt == n2:
                            match_end_winner = second_team_name
                            break
            ...
            try:
                try:
                    base_idx = parts.index("objective_captured")
                except ValueError:
                    base_idx = parts.index("point_captured")
            ...
            try:
                ...
                def get_val(idx):
                    try:
                        return float(parts[base_idx + idx])
                    except Exception:
                        return 0.0
                ...
                stat_entry = {
                    ...
                    "player_name": p_name,
                    "hero_name": p_hero_kr,
                    "hero_image": p_hero_en,
                    "slot_index": -1,
                    "eliminations": get_val(6),
                    "final_blows": get_val(7),
                    "deaths": get_val(8),
                    "all_damage_dealt": get_val(9),
                    "barrier_damage_dealt": get_val(10),
                    "hero_damage_dealt": get_val(11),
                    "healing_dealt": get_val(12),
                    "healing_received": get_val(13),
                    "self_healing": get_val(14),
                    "damage_taken": get_val(15),
                    "damage_blocked": get_val(16),
                    "defensive_assists": get_val(17),
                    "offensive_assists": get_val(18),
                    "ultimates_earned": get_val(19),
                    "ultimates_used": get_val(20),
    ...
    for r in valid_round_nums:
        ...
        for t_name, entries in team_players.items():
            ...
            for p_name, p_entries in unassigned_players:
                ...
                preferred_slots = []
                if role_score == 0:
                    preferred_slots = [0, 1, 2, 3, 4]
                elif role_score == 1:
                    preferred_slots = [1, 2, 0, 3, 4]
                else:
                    preferred_slots = [3, 4, 1, 2, 0]
                ...
                if assigned_slot == -1:
                    assigned_slot = 5
                    while assigned_slot in used_slots:
                        assigned_slot += 1
...
def calculate_pure_stats(parsed, target_match):
    ...
    for r_data in round_scores.values():
        ...
        if r_w == n_team1:
            score_round_wins_t1 += 1
        elif r_w == n_team2:
            score_round_wins_t2 += 1
        ...
        for ev in parsed["events"]:
            if ev.get("event_type") == "objective_updated" and ev.get("round") == r_num:
                ...
                if idx > max_idx:
                    max_idx = idx
        if attacker == n_team1:
            score_obj_t1 += max_idx
        elif attacker == n_team2:
            score_obj_t2 += max_idx

    is_push = any(
        k in map_name for k in ["밀기", "Push", "에스페란사", "이스페란사", "뉴 퀸", "콜로세오", "룬아사피", "루나사피"]
    )
        ...
        if match.pauses and len(match.pauses) > 0:
            for p in match.pauses:
                ...
                if s_sec > 0 and e_sec > 0 and s_sec != e_sec:
                    ...
                    processed_pauses.append({"start_sec": s_sec, "end_sec": e_sec, "duration": e_sec - s_sec})
